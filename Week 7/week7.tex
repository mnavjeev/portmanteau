...
\begin{proof}
	...
	\end{align*}
	Note the argument that allows us to replace the first outer expectation with iterated expectations over \(X\) and  \(\eps\): each of the functions in \(\calF\) are measurable and  \(\calF_M\) is bounded, which means that the supremum will be measurable and bounded with probability 1 in any finite sample (with respect to the empirical measure/conditional on the \(X\) data).
	...
	for any \(C\) with  \(\E\left[\exp\left(\frac{f(X_i)}{C^2}\right)-1\right]\leq 1\). In our setting we will apply this to the functions \(\frac{1}{n}\sum_{i=1}^n \eps_i g(X_i)\) for \(g\in\calG_\delta\), with the \(g(X_i)\) treated as fixed so that  these are considered random variables in \(\eps_i\). In our setting we can bound:
	\[
		\E_\eps\left[\sup_{g\in\calG_\delta}\bigg|\frac{1}{n}\sum_{i=1}^n \eps_ig(X_i)\bigg|\right] \leq C \sqrt{1 + \log\calN\left(\delta,\calF_M,\|\cdot\|_{L_1(\P_n)}\right)}
	.\] 
	for such a \(C\) such that 
	 \[
		 \E_\eps\left[\exp\left(\bigg(\frac{1}{n}\sum_{i=1}^n \eps_if(X_i)\bigg)^2/C^2\right)-1\right]\leq 1
	.\] 
\end{proof}
